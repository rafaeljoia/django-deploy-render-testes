...
class Command(BaseCommand):
    help = 'Creates a de franco.'

    def fetch_property_data(self, url, headers):
        try:
    ...
    def parse_property_data(self, soup):
        properties = []
        ...
        for div_imovel in divs_imoveis:
            try:
                ...
                bairro_div = div_imovel.find("div", class_="property-content").find("p",
                                                                                    class_="location").text.strip().replace(
                    "Bairro", "").strip()
                property_data = {
                    "Valor": valor_div,
                    "Tipo": tipo_div,
    ...
    def start_collect(self):
        ...
        soup = self.fetch_property_data(url, headers)
        if soup:
            properties = self.parse_property_data(soup)
            self.save_property_data(properties)
        else:
