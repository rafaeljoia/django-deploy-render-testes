...
class Command(BaseCommand):
    help = 'Creates a Pedrao'

    def fetch_property_data( self, url, headers, payload):
        try:
    ...
    def parse_property_data(self, dados):
        properties = []
    ...
    def start_collect(self):
        ...
        while has_data:
            ...
            dados = self.fetch_property_data(url, headers, payload)
            if dados:
                TOTAL_PAGE = int(dados['totalItens']) // 12

                if TOTAL_PAGE > PAGE_INDEX:
                    result.extend(self.parse_property_data(dados))
                    PAGE_INDEX += 1
        ...
        self.save_property_data(result)
